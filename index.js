...
async function processEvents(events) {
  ...
  for (let i = 0; i < events.length; i++) {
    ...
    } else if (event == "SupportWalletFund") {
      try {
        
          const iswit = await supportwallet.create({  
          user: returnValues.user,
          amount: returnValues.amount,
          polAmt : returnValues.polAmt,
          txHash: transactionHash,
        ...
        console.log("Error (SupportWalletFund Event) :", e.message);
      }
    } else if (event == "GrowthWalletFund") {
      try {
        
          const iswit = await growthwallet.create({  
          user: returnValues.user,
          amount: returnValues.amount,
          polAmt : returnValues.polAmt,
          txHash: transactionHash,
        ...
        console.log("Error (GrowthWalletFund Event) :", e.message);
      }
    } else if (event == "GivewayWalletFund") {
      try {
        
          const iswit = await giveawaywallet.create({  
          user: returnValues.user,
          amount: returnValues.amount,
          polAmt : returnValues.polAmt,
          txHash: transactionHash,
        ...
        console.log("Error (GivewayWalletFund Event) :", e.message);
      }
    }
  }
}
